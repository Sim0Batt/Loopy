...
object QueryManagement {
    ...
    fun getDatas(connection: Database): ReturnDataJson {
        ...
        transaction(connection) {
            //PPG
            val hrTmp = TabellaPpgTable.selectAll().orderBy(TabellaPpgTable.id to SortOrder.DESC).map{
                it[TabellaPpgTable.battito]
            }.take(10)
            val oxTmp = TabellaPpgTable.selectAll().orderBy(TabellaPpgTable.id to SortOrder.DESC).map{
                it[TabellaPpgTable.ossigenazione].replace("%", "").toDouble()
            }.take(10)
            var tsTmp = TabellaPpgTable.selectAll().orderBy(TabellaPpgTable.id to SortOrder.DESC).map{
            ...
            //Electrodes
            val swTmp = TabellaElettrodiTable.selectAll().orderBy(TabellaElettrodiTable.id to SortOrder.DESC).map{
                it[TabellaElettrodiTable.sudorazione]
            }.take(10)

            tsTmp = TabellaElettrodiTable.selectAll().orderBy(TabellaElettrodiTable.id to SortOrder.DESC).map{
            ...
            //Termometer
            val trTmp = TabellaTermometroTable.selectAll().orderBy(TabellaTermometroTable.id to SortOrder.DESC).map{
                it[TabellaTermometroTable.temperatura]
            }.take(10)

            tsTmp = TabellaTermometroTable.selectAll().orderBy(TabellaTermometroTable.id to SortOrder.DESC).map{
