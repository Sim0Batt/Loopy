...
class MainActivity : BaseActivity() {
    ...
    private suspend fun downloadImagesBitmap(client: HttpClient, userId: Int): List<Bitmap> {
        ...
        try {
            var serverResponse = client.get("http://13.53.132.231:8080/generateGraph/stress/$userId")
            var imageBytes = serverResponse.readBytes()
            tmp.add(BitmapFactory.decodeByteArray(imageBytes, 0, imageBytes.size))

            serverResponse = client.get("http://13.53.132.231:8080/generateGraph/sleep/$userId")
            imageBytes = serverResponse.readBytes()
            tmp.add(BitmapFactory.decodeByteArray(imageBytes, 0, imageBytes.size))

            serverResponse = client.get("http://13.53.132.231:8080/generateGraph/activity/$userId")
            imageBytes = serverResponse.readBytes()
            tmp.add(BitmapFactory.decodeByteArray(imageBytes, 0, imageBytes.size))
        }catch (e: Exception){
